...
def generate_image():
    xt = torch.randn(4,4,64,64).to(device)
    ...
    with torch.no_grad():
        xt = xt / vae.config.scaling_factor
        xt = vae.decode(xt.to(device)).sample
...
# The optimizer
opt = torch.optim.AdamW(unet.parameters(), lr=5e-4) 
